...
public class Tasks {
    ...
    public final static String[][] columnTypes = {
            ...
            {"INTEGER", "INTEGER", "INTEGER"},
             /* 4 */ {"INTEGER", "INTEGER", "VARCHAR(100)"},
            {"INTEGER", "INTEGER", "VARCHAR(100)", "INTEGER", "INTEGER"}

    };
    public final static String[] tableNames = {"tasks", "reminder", "alarm", "repeat", "notes", "subtasks"};
    public final static int[] columnNos = {9, 4, 6, 3, 3, 5};
    toast maketext;
    ...
    public static void insert(String[] passDatas, int[] passInt,
        ...
        DbChangeReceiver.broadCastDbChange(context, IntentActions.ACTION_TASK_INSERT, new int[]{tid, passInt[1], passInt[4]}, passDatas[1], null);
    ...
    public static void update(String[] passDatas, int[] passInt, Context context,
        ...
        //DBProvider dbProvider =new DBProvider();
        Log.d("tasks"," "+passDatas.toString());
        String task = passDatas[0];
        String selection = columnNames[0][0] + " = " + tid;
        ContentValues contentTask = new ContentValues();

        /**
         * 0- isrem 1- istime, 2- timehr, 3- timemin, 4- isalarm, 5-isrepeat, 6-repeatmode
         */
        int[] remData = {passInt[1], passInt[5], passInt[6], passInt[7], passInt[8], passInt[9], passInt[10]};
        int notification_update_mode = TaskUtil.getNotificationUpdate(context, remData, passDatas[2], tid);

        Log.d("tasks","notification update mode "+notification_update_mode);

        if (notification_update_mode != 0)
            passInt[4] = 0; //done = 0
        else
            passInt[4] = LoadTaskHelper.getDone(context, tid); //no change to done

        int old_pid = LoadTaskHelper.getProject(context, tid);
        if (old_pid != passInt[0])
        ...
        DbChangeReceiver.broadCastDbChange(context,
                IntentActions.ACTION_TASK_UPDATE,
                new int[]{tid, passInt[1], passInt[4]}, passDatas[1],
                new int[]{notification_update_mode});
